...
if !issiso(sys)
  error("Only monovariable systems allowed");
endif
...
if isempty(Ts) || Ts > 0
  error("Only continuous systems allowed");
endif
...
if Np_imag
    impoles = impoles(end+1-Np_imag:end);
end
...
% to decide radius for half circles around imaginary poles;
eps = 1e-3;
R = eps;
...
circle(0,0.5,'-');
circle(0,0.6666666667,'-.');
circle(0,0.833333333,'-.');
circle(0,1,'-', 'LineWidth', 1.25, 'Color', [0.55 0.55 0.55]);
circle(0,1.16666667,'-.');
circle(0,1.33333333,'-.');
circle(0,1.5,'-');
phase_lines(24, 0.5, 1.5, '-.');
phase_lines(8, 0.5, 1.5, '-');
...
if npoles_on_im_axis > 0
    ...
    if closedloop_rhp_poles > 0
        ...
        fprintf(1,'=> Closed-loop-system is asymptotically stable\n');
    end
end
...
text(0.8, -0.64,'0 dB','FontSize',16);
...
scalexy = axis; scalexy(3:4) = 1.00*scalexy(3:4);
axis(scalexy); 
...
%***********************************************
function circle(zcentre, radius, varargin)
...
%***********************************************
function phase_lines(n,rstart,rend,varargin)
hold on;
...
for k = 1 : n
    ...
    lines(k,1) = rstart*zh;
    lines(k,2) = rend*zh;
    plot(real(lines(k,:)), imag(lines(k,:)), 
        'LineWidth', 1.0,
        'Color', [0.5 0.5 0.5],
        varargin{:});
end
...
%***********************************************
function val = evalfr(sys, x)
    % Local replacement for evalfr(sys, x) 
    % Works for SISO transfer functions 
    [num, den] = tfdata(sys, "vector");
    numv = polyval(num, x);
    denv = polyval(den, x);
    val = numv / denv;
...
function [zplot, ncount] = nygraph(sys, s, varargin)
...
for k = 3 : length(zall)
    ...
    if closest_now && min([dl1 dl2 dl3]) < 1e-5 && min([abz1 abz2 abz3]) < 0.001
    ...
    if zre1 < -0.9
        ...
        if zim1*zim2 < 0
            ...
            delta12 = (real(z1) - real(z2)) * abs(imag(z2)) / (abs(imag(z1))
                        + abs(imag(z2)));
...
for k = 1 : n
   ...
   plot(cplot,'-','LineWidth',1.0);
   % plot(cplot,'-','LineWidth',1.0, 'Color', [1.0 1.0 1.0]);
end
endfunction
